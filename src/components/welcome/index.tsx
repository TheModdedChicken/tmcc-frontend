...
function Welcome(props: {setUsername: (username: string) => void, setRoom: (room: string) => void, connect: () => void}) {
  return (
    <>
      ...
      <TextBox id="room-textbox" name="Room" infoText="(Max length 30)" placeholder="Enter a room name... (i.e. general)" maxLength={30} onChange={(e) => {if (RoomNameRegex.test(e.target.value)) props.setRoom(e.target.value) }}></TextBox>
